...
col_inter = ['Country', 'Region', 'Tectonic regime',
       ...
       'Structural setting', 'Trapping mechanism', 'Depth']
repeat = 10
for large in range(10, len(col_inter)):
#for large in range(10, 11):
    for iter_sample in range(repeat):
        ...
        with open(f'{currentdir}/log_full.txt', 'a', encoding="utf-8") as log:
            log.write(f'{large}\n')
        ...
        #for alpha in [0.5]:
            ...
            time_long = time.time() - time_start
            with open(f'{currentdir}/log_coded.txt', 'a', encoding="utf-8") as log:
                log.write(f'{large}\n')
                log.write(f'{alpha}\n')
                log.write(f'Sample {col_new}\n')
                log.write(f'Time {time_long}\n')
